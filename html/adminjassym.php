...
if ($result->num_rows > 0) {
    ...
    echo '<form method="POST">';
    echo '<h2 for="competence_select">Sélectionnez un evenement à supprimer :</h2>';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['deletecompetence']) && isset($_POST['competence_id'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            echo "<p class='success'>L'evenement a été supprimé avec succès.</p>";
        } else {
            echo '<p class="error">Erreur lors de la suppression : ' . $stmt->error . '</p>';
        }
        $stmt->close();
